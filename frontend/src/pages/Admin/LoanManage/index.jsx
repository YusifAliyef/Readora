...
function AdminLoansManage() {
  ...
  const handleReturnConfirm = async () => {
    ...
    try {
      const res = await axios.put(
        `${API_URL}/loans/${returnTarget._id}/return`,
        {},
        authHeaders,
      );
      ...
      toast.success(
        fine > 0
          ? `Qaytarıldı — cərimə: ${fine} AZN`
          : "Kitab uğurla qaytarıldı",
      );
      fetchLoans();
    } catch (err) {
      toast.dismiss(loadingToast);
  ...
  const getStatusBadge = (loan) => {
    if (loan.status === "returned") {
      return (
        <span className={`${styles.badge} ${styles.returned}`}>Qaytarılıb</span>
      );
    }
    const isOverdue = new Date() > new Date(loan.dueDate);
    if (isOverdue) {
      return (
        <span className={`${styles.badge} ${styles.overdue}`}>Gecikib</span>
      );
    }
    ...
    <div className={styles.page}>
      <div className={styles.header}>
        ...
        <div className={styles.filters}>
          ...
          <button
            className={
              filter === "borrowed" ? styles.filterActive : styles.filterBtn
            }
          ...
          <button
            className={
              filter === "returned" ? styles.filterActive : styles.filterBtn
            }
          ...
          <table className={styles.table}>
            ...
            <tbody>
              {loans.map((loan) => (
                <tr key={loan._id}>
                  <td>
                    <strong>{loan.book?.title || "Bilinməyən Kitab"}</strong>
                  </td>
                  <td>
                    {loan.user?.fullName || loan.user?.userName || "Naməlum"}
                  </td>
      ...
      {returnTarget && (
        ...
        >
          ...
          >
            ...
            <p>
              <strong>"{returnTarget.book?.title}"</strong> kitabının
              qaytarıldığını təsdiqləyirsiniz? Gecikmə varsa, cərimə avtomatik
              hesablanacaq.
            </p>
